...
#for(s in 1:10){
  ...
  x_test <- GDSC$x[-trainID,(num.nonpen+1):(num.nonpen+sum(p))]
  x <- GDSC$x[trainID,(num.nonpen+1):(num.nonpen+sum(p))]
  y_test <- GDSC$y[-trainID,]
  y <- GDSC$y[trainID,]

  x_test[,1:p[1]] <- log(GDSC$x[-trainID, (num.nonpen+1):(num.nonpen+p[1])])
  x[,1:p[1]] <- log(GDSC$x[trainID, (num.nonpen+1):(num.nonpen+p[1])])
  ...
  for (k in 1:nfolds){ 
    ...
    X1 = scale(x[indx,1:p[1]])
    X2 = scale(x[indx,(p[1]+1):(p[1]+p[2])])
    X3 = x[indx,(p[1]+p[2]+1):sum(p)]
    Xlist = list(X1,X2,X3)
    ...
    X1 = scale(x[ind,1:p[1]])
    X2 = scale(x[ind,(p[1]+1):(p[1]+p[2])])
    X3 = x[ind,(p[1]+p[2]+1):sum(p)]
    X_test = cbind(X1,X2,X3)
    for (l in 1:length(lambda.seq)){
      fit <- iRRR(y[indx,],X=Xlist,lam1 = lambda.seq[l],paramstruct = list(maxrho = 500,varyrho=1,noreg = c(0),
                                                                           rho = 0.0001,
                                                                           Niter = 2000,
                                                                           weight = ww,
                                                                           Tol = 1e-6))
      ...
      print(paste("MSPE:",round(MSPE,4),"corr:",round(corr,4),"rank X1:",rankMatrix(fit$A[[1]]),
      "rank X2:",rankMatrix(fit$A[[2]]),"rank X3:",rankMatrix(fit$A[[3]])))
  ...
  # Setting up data
  X1 = scale(x[,1:p[1]])
  X2 = scale(x[,(p[1]+1):(p[1]+p[2])])
  X3 = x[,(p[1]+p[2]+1):sum(p)]
  Xlist = list(X1,X2,X3)
  #Xlist.GLR = list(X0,cbind(X1,X2,X3))
  ww = rep(NA,length(Xlist))
  for (i in 1:length(Xlist)){
    ww[i] = svd(Xlist[[i]])$d[1]*(sqrt(q) + sqrt(as.numeric(rankMatrix(Xlist[[i]]))))/dim(Xlist[[i]])[1]
  }
  X1_test = scale(x_test[,1:(p[1])])
  X2_test = scale(x_test[,(p[1]+1):(p[1]+p[2])])
  X3_test = x_test[,(p[1]+p[2]+1):sum(p)]
  X_test = cbind(X1_test,X2_test,X3_test)

  # Local Low Rank
  fit <- iRRR(y,X=Xlist,lam1 = lam1.cvOpt,paramstruct = list(maxrho = 500,varyrho=1,noreg = c(0),
                                                                            rho = 0.0001,
                                                                            Niter = 2000,
                                                                            weight = ww,
                                                                            Tol = 1e-6))
  ...
  print(paste("MSPE:",round(MSPE,4),"corr:",round(corr,4),"rank X1:",rankMatrix(fit$A[[1]]),
              "rank X2:",rankMatrix(fit$A[[2]]),
              "rank X3:",rankMatrix(fit$A[[3]])))
  ...
  for (i in 1:length(alpha.seq)){
    cvGLMNET = cv.glmnet(x,y,alpha=alpha.seq[i],nfolds=5,foldid=foldid,family="mgaussian")
...
#}
  ...
  save.image(file=paste0("results_pen_s=",MultSeed[s],".Rdata"))  
